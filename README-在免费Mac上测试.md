...
| 脚本 | 它自己打印的汇总行（**认这一行**） |
|---|---|
| `_verify_mac_fonts.py` | `通过 41 项，失败 0 项` |
| `_verify_mac_format.py` | `通过 132 项，失败 0 项，跳过 5 项` ※ |
| `_verify_mac_platform.py` | `通过 51 项，失败 0 项，跳过 5 项` |
| `_verify_runner_layout.py` | `通过 3 个脚本，失败 0 个`（它自己还会补一句：共跳过 15 条） |

> ※ **format 的项数会随断言增加而变大**（2026-09-16 修完卡死后 +7 → 约 139）。
> 所以**别死记数字**，认「`失败 0 项`」和「`=== 全部通过 ===`」这两处就够。
> `_verify_runner_layout.py` 会把子脚本的输出也带出来，里面那些
> `通过 134 项` 之类是**子脚本**的，不是它自己的 —— 看它最后那行 `通过 3 个脚本`。

> ⚠️ **别拿本机的数字去对 CI 的日志 —— 对不上是正常的，那不代表出问题。**
>
> 原因有两个，都跟「本机环境 ≠ runner 环境」有关：
>
> 1. **布局不同**：本机上游 `D:\video-tool\` 有 Windows 侧文件
>    （`配置git.bat`、`_push_mac.py` 等），**仓库里只有 `mac/`** ——
>    与这些文件相关的断言在 CI 上会整体跳过，所以通过数变少。
> 2. **计数口径不同**：有的脚本同一批跳过项会打印两遍（实时一条 + 末尾汇总一条），
>    自己数 `SKIP` 开头的行会翻倍；`_verify_runner_layout.py` 还会把子脚本的
>    汇总行一起打出来。**要读脚本自己那行汇总**（`通过 N 项，失败 N 项，跳过 N 项`），
>    别自己数行。
>
> **所以：上面表里的「通过」数是 run #5 的实际值，仅供参考。
> 真正要看的只有两件事 —— 每个脚本 `exit=0`，且 `FAIL` 行 = 0 条。**

**一句话判据（最重要，记这一条就够）：**

> 第 5 步正常时，四个脚本**全部 `exit=0`**，`FAIL` 行应为 **0 条**。
> **只要出现 `FAIL`，就是真问题；`SKIP` 是正常的，不用管。**
> 「通过」数比表里多几项或少几项都**不用理** —— 那是环境差异，不是回归。
...
**Q：能不能在 Windows 上下载、解压，再把 `.app` 拷到 Mac？**
A：**下载可以，解压不行。**

| 动作 | Windows | Mac |
|---|---|---|
| 下载这个 zip | 可以（zip 就是个普通文件） | 可以（最省事，直接在 Mac 上下） |
| **解压**这个 zip | **不要** | **必须在这儿** |

**为什么解压必须回 Mac —— 两个硬理由：**

1. **可执行权限位会丢。** zip 把 Unix 权限存在「外部属性」字段里，而 Windows 的解压工具
   （包括资源管理器自带的「全部解压缩」、以及多数第三方工具）**不还原这些位**。
   偏偏 `.app` 就是靠它们运行 —— `video_tool_macos.spec` 末尾专门写了一段
   `chmod 755`，给 `.app` **内置的 ffmpeg / ffprobe** 补可执行位，就是因为这个位很脆。
   丢了之后的现象是「已损坏，无法打开」或双击毫无反应 —— 很容易误判成包打坏了。
2. **符号链接会坏。** PyInstaller 打出来的 macOS 包里含符号链接，
   Windows 解压会把它们变成空文件、或直接报错 → 包里结构断裂 → 打不开。

**正确做法（二选一）：**

- **手边有 Mac** → 直接在 Mac 上打开 Actions 页面下载，Finder 里双击解压。最省事。
- **只有 Windows** → 在 Windows 上**下载但不要解压**，把 zip **原封不动**传过去
  （微信/QQ 传文件、网盘、U 盘都行），到 Mac 上再解压。

> 🔴 一句话：**zip 可以让 Windows 经手，但「解压」这一步必须发生在 Mac 上。**

> 另外：下载来的文件会被 macOS 打上「隔离」标记，首次打开要右键 →「打开」→ 再点「打开」。
> 实在被拦就在 Mac 终端跑一次：`xattr -dr com.apple.quarantine /路径/视频工具.app`
...
| 显示 | 被掐那步的**耗时** | 含义 | 怎么办 |
|---|---|---|---|
| `cancelled`（灰/黑 ✗） | 几秒 ~ 几分钟 | 被人为打断（点了 Cancel / 重复点 Run） | 直接**重新跑一次** |
| `cancelled`（灰/黑 ✗） | **≈ 45 分钟（卡满上限）** | **卡死被 `timeout-minutes` 掐掉** | ⚠️ **别直接重跑！重跑还是同一处**，先下「打包日志」看 |
| `failure`（红 ✗） | —— | **真失败**，有根因 | 下载「自检详细日志」发我 |

**怎么分辨**：点进那次运行，看每个步骤的标记**和耗时** ——
...
- **关键看那一步跑了多久**：几分钟就灰 → 人为打断；**卡到 44~45 分钟** → 是撞超时。

> 🔴 **2026-09-16 实例（重要，我一开始判错了，害你白跑一轮）**：
> run #5 和 run #6 **两次都是 `cancelled`，而且都掐在同一个地方** ——
> 「打包」那步分别跑了 **44.4 / 44.8 分钟**，正好撞上 workflow 里的
> `timeout-minutes: 45`。**这不是「有人点了取消」，是卡死被超时掐掉的。**
>
> 真根因：`app.py` 的自检跑完会弹一个**模态** `QMessageBox` 等人点「确定」——
> CI 上没人点 → **永久阻塞** → 一直干等到 45 分钟上限。
> 注意 `QT_QPA_PLATFORM=offscreen` 只解决「显示」，**不解决「阻塞」**，挡不住这个坑。
> （`.app` 其实早就打好了 —— 日志里能看到 `✓ 打包完成`，只是卡在后面的自检上。）
>
> **已修**：① 打包脚本 + workflow 都设 `VIDEOTOOL_SELFTEST_NOGUI=1`；
> ② `app.py` 自检会自己判断「非交互环境」并跳过弹窗；
> ③ 打包脚本加了 300 秒看门狗，绝不会再无限期干等；
> ④ `timeout-minutes` 45 → 90；⑤ 加 `PYTHONUNBUFFERED=1`，卡住也能从日志看到跑到哪。
>
> **口诀：`cancelled` ≠「有人点了取消」。先看被掐那一步的耗时 —— 卡满 45 分钟就是超时，
> 重跑一百次也一样，必须改代码。**

**Q：`烧中文字幕` 那一步红了，我的 `.app` 还能拿到吗？**
A：**能。** 2026-09-17 起，「上传 .app 成品」排在「烧中文字幕」**之前**了 ——
那一步只是**验证**（还设了 `continue-on-error: true`），它红了不影响产物。

> 🔴 这一条是踩出来的：run #7 里「烧中文字幕」2 秒就挂了，把后面的
> 「打包 zip / 上传 .app」**全带成 `skipped`** —— `.app` 明明打好了却下不到，
> 白等一轮。**教训：交付物步骤必须排在验证步骤之前。**

它红了先看 **`打包日志`** artifact —— 现在里面会带 `/tmp/burn.log`，
有「字体候选 存在/缺失」的逐条清单 + `subtitle.available_fonts()` 的实际输出。

**Q：自检报告里写 `模块 fontTools.ttLib / freetype **失败**`，要紧吗？**
A：**要紧，但程序不会崩。** 这两个包管的是字幕的**精确度量**：
`fontTools` 读字体的 OS/2 度量、`freetype-py` 量字形实际 ink 高度。
缺了以后 `subtitle_tab.py` 里那两处 `import` 会走 `except` 分支，
**静默降级**成经验公式估算 —— 不报错，但排版精度变差（最难受的那种 bug）。

> 已修（2026-09-17）：`打包mac.sh` 与 workflow 的依赖清单都加上了
> `fontTools freetype-py`。以后自检报告里这两行应该是 `OK`。
